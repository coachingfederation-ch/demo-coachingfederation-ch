...
is *not* translated — those come from the `cf_*` vocabularies, which already
...
In `/my-profile` → *Profile languages* (`ProfileTranslationsPanel`):
...
Directory *search* deliberately matches primary-language text only.
