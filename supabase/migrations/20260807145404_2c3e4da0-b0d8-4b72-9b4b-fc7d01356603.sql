-- Expose the owning local community on the public events projection so the
-- community detail page can list its upcoming events. Additive only: same
-- rows, same published-only filter, same security_invoker semantics.
