 ...
 * the one piece of markup every translation panel (articles, events,
 * profiles) shares, so the locale-tab structure only exists once.
 */
    ...
    <>
      {items.map((item) => (
        <div
          key={item.locale}
          className={
            item.rowClassName ?? "border-t border-border pt-3 first:border-0 first:pt-0"
          }
        >
          ...
          {item.hint ?? null}
          <div className={item.actionsClassName ?? "mt-2 flex items-center gap-2"}>
            <button
              type="button"
              onClick={item.onTranslate}
              disabled={item.translateDisabled}
              title={item.translateTitle}
